...
Int_t GEMBaseValidation::getDetOccBinX(Int_t num_chambers, Int_t chamber_id, Int_t layer_id) {
  return num_chambers * chamber_id + layer_id - num_chambers;
}
...
MonitorElement* GEMBaseValidation::bookZROccupancy(DQMStore::IBooker& booker,
  ...
  for (Double_t i = station0_xmin - 1; i < station2_xmax + 1; i += 0.5) {
